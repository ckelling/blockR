# Functions taken from the maptools package (now deprecated)
# https://maptools.r-forge.r-project.org/reference/maptools-deprecated.html

#Source: https://github.com/cran/maptools/blob/master/R/union.R
#adapted to no longer depend on rgeos

unionSpatialPolygons_pck <- function(SpP, IDs, threshold=NULL) {
...
################################################ 
#### Source: https://github.com/cran/maptools/blob/master/R/elide.R
################################################ 
...
elide.polygonsdf <- function(obj, bb=NULL, shift=c(0, 0),
  ...
  res <- elide(as(obj, "SpatialPolygons"), bb=bb, shift=shift,
               reflect=reflect, scale=scale, flip=flip, rotate=rotate, center=center)
  df <- as(obj, "data.frame")
  res <- SpatialPolygonsDataFrame(res, data=df, match.ID = FALSE)
  res
}
